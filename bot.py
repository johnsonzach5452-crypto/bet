...
def _apply_data_sheet_formatting(ss, ws):
    ...
    # ── Column widths ───────────────────────────────────────────────────
    widths = [140,140,100,80,300,100,70,110,90,130,55,70,80,80,75,85,70,120,70,160,80,90,75]
...
- SEASONS MATTER: the WNBA plays May–October; the NBA plays October–June. A basketball bet placed in July, August, or September is almost certainly WNBA. Check team names: Aces/Liberty/Lynx/Storm/Sun/Mercury/Sparks/Fever/Sky/Wings/Mystics/Dream/Valkyries are WNBA
...
async def on_ready():
    ...
    if ODDS_API_KEY and not score_watch.is_running():
        score_watch.start()
        log.info(f"score_watch started (every {SCORE_POLL_MINUTES}m, auto-grade={'on' if AUTO_GRADE else 'off'})")
    ...
    for g in cands:
        ...
        if btype == "moneyline":
            if both_named:   # ambiguous side -> don't track
                return None, None
            team = home if (hf and hf in desc) else away
...
_WNBA_TEAMS = ["aces","liberty","lynx","storm","sun","mercury","sparks",
    "fever","sky","wings","mystics","dream","valkyries"]


def _basketball_league_from_desc(desc):
    """Return 'NBA', 'WNBA', or None (no/ambiguous team evidence)."""
    d = (desc or "").lower()
    nba = any(re.search(rf"\b{t}\b", d) for t in _NBA_TEAMS)
    wnba = any(re.search(rf"\b{t}\b", d) for t in _WNBA_TEAMS)
    if nba and not wnba:
        return "NBA"
    if wnba and not nba:
        return "WNBA"
...
# ═══════════════════════════════════════════════════════════════════════
# 10-min polling grades bets within ~0-10 min of a final. Quota math:
# scores call = 2 credits/sport; only fires while game-level bets are pending.
# Worst case ~17K credits/mo, and $0 on nights you have no game bets pending.
...
def _desc_matches_events(desc, events):
    d = (desc or "").lower()
    for e in events:
        if _team_frag(e.get("home_team")) in d or _team_frag(e.get("away_team")) in d:
            return True
    return False
    ...
    async def _run():
        ...
        for (sk, day), day_bets in sorted(by_day.items()):
            if credits["used"] >= max_credits:
                break
            ev_resp = await loop.run_in_executor(None, _hget,
                f"https://api.the-odds-api.com/v4/historical/sports/{sk}/events",
                {"date": f"{day}T12:00:00Z"})
            ev_list = (ev_resp or {}).get("data", []) if isinstance(ev_resp, dict) else (ev_resp or [])
            if not ev_list:
                failed += len(day_bets)
                continue
            ...
            for b in day_bets:
                ...
                cands = [e for e in ev_list
                         if _team_frag(e.get("home_team")) in desc
                         or _team_frag(e.get("away_team")) in desc]
                if len(cands) != 1:
                    failed += 1
                    continue
                ev = cands[0]
                ...
                if eid not in odds_cache:
                    odds_cache[eid] = await loop.run_in_executor(None, _hget,
                ...
                if btype == "moneyline" and not both:
                    ...
                    price = _consensus_price(data, "h2h", team)
                elif btype == "total":
                    m = _TOTAL_RE.search(desc)
                    if m:
                        price = _consensus_price(data, "totals",
                                                 (m.group(1).capitalize(), float(m.group(2))))
                elif btype == "spread" and not both:
                    m = _SPREAD_RE.search(b.get("description") or "")
                    if m:
                        team = ev["home_team"] if hf in desc else ev["away_team"]
                        price = _consensus_price(data, "spreads", (team, float(m.group(1))))
                if price is None:
                    failed += 1
                    continue
        ...
        summary = (f"**🕰️ CLV Backfill Complete**\n"
                   f"✅ {done} closing lines captured  ·  ⏭️ {failed} skipped (strict rules)\n"
                   f"💳 ~{credits['used']} credits used\n"
                   f"Run `/resync` to push CLV to the sheet, then check the dashboard.")
        for grp in GROUPS:
            try:
...
async def fixleagues(interaction: discord.Interaction):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        cur = await db.execute(
            "SELECT message_id, description, league, created_at FROM bets "
            "WHERE lower(COALESCE(sport,''))='basketball' "
            "OR upper(COALESCE(league,'')) IN ('NBA','WNBA')")
        ...
        for b in rows:
            ...
            if correct and claimed != correct:
                await db.execute(
                    "UPDATE bets SET league=?, sport='Basketball' WHERE message_id=?",
                    (correct, b["message_id"]))
                changes.append(f"{claimed or '?'} → {correct}: {(b.get('description') or '')[:55]}")
        await db.commit()
