...
_SHEETS_RATE_LIMIT = int(os.environ.get("SHEETS_RATE_LIMIT_PER_MIN", "50"))
_SHEETS_RATE_WINDOW = 60.0
_sheets_call_times = deque()
_sheets_throttle_lock = asyncio.Lock()


async def _call_sheets(fn):
    """Run a Sheets function on the executor. Waits for a free slot in the
    rolling window before dispatching, so a burst of bets queues smoothly
    instead of firing all at once and hitting a 429."""
    ...
    async with _sheets_throttle_lock:
        while True:
            ...
            if len(_sheets_call_times) < _SHEETS_RATE_LIMIT:
                _sheets_call_times.append(now)
                break
            # window is full — wait until the oldest call ages out
            sleep_for = _SHEETS_RATE_WINDOW - (now - _sheets_call_times[0]) + 0.05
            log.info(f"Sheets rate window full ({len(_sheets_call_times)}/"
                    f"{_SHEETS_RATE_LIMIT} in {_SHEETS_RATE_WINDOW:.0f}s) — "
                    f"queuing next write for {sleep_for:.1f}s")
...
async def sheets_update_odds(username, message_id, live=None, closing=None, clv=None,
                             game_time=None, player_img=None):
...
def verify_league(data):
    ...
    try:
        ...
        # ── FALLBACK: weaker heuristics, only when no schedule game had
        # both teams named (API down, off-season, or genuinely no schedule
        # evidence either way). These are unchanged from before. ──────
    ...
    async def _do_resync():
        ...
        try:
            ...
            if not SHEETS_AVAILABLE or not SPREADSHEET_ID:
                ...
                await _call_sheets(_rewrite_sheets)
