...
def _espn_logo_url(description, sport=None, league=None):
    ...
    Returns a URL or None; None simply means no thumbnail on the card."""
    raw = description or ""
    d = raw.lower()
    ...
    for nick, ref in _ESPN_LOGO.items():
        ...
        if i >= 0 and i < best_pos:
            best, best_pos = ref, i
    if not best:
        # fall back to abbreviations, scoped by sport so shared codes are safe
        lg = (league or "").upper().strip()
        slug = ("wnba" if lg == "WNBA" else "nba" if lg == "NBA"
                else "mlb" if lg == "MLB" or "baseball" in (sport or "").lower()
                else None)
        if slug == "mlb":
