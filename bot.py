...
@bot.tree.command(name="sheetsync", description="Compare DB vs Google Sheet and remove any orphaned rows from the sheet")
async def sheetsync(interaction: discord.Interaction):
    ...
    # Get all message IDs from the DB for this group
    async with aiosqlite.connect(DB_PATH) as db:
        query = "SELECT message_id, username, description FROM bets"
        params = []
        if group_name:
    ...
    db_message_ids = {str(r[0]): {"username": r[1], "description": r[2]} for r in db_rows}
    ...
    def _check():
        ...
        # Check every data tab (not dashboard/calendar tabs)
        for ws in ss.worksheets():
            ...
            msg_id_col = ws.col_values(COL_MSG_ID)  # column T
            for row_idx, msg_id in enumerate(msg_id_col[1:], start=2):  # skip header
                if not msg_id:
                    continue
                if str(msg_id) not in db_message_ids:
                    # This row is in the sheet but NOT in the DB — it's orphaned
                    desc = ws.cell(row_idx, 5).value or "unknown"  # column E = description
                    orphaned.append({
                        "username": username,
                        "row": row_idx,
                        "msg_id": msg_id,
        ...
        # Check DB bets that are missing from the sheet (won't auto-fix these, just report)
        for msg_id, info in db_message_ids.items():
            uname = info["username"]
            try:
                ws = ss.worksheet(uname)
                if _find_row_by_message_id(ws, msg_id) is None:
                    missing.append(f"{info['description'] or 'unknown'} (@{uname})")
            except Exception:
                pass

    await loop.run_in_executor(None, lambda: _sheets_call(_check))

    # Delete orphaned rows from sheet (in reverse order to preserve indices)
    deleted = []
    if orphaned:
        def _delete_orphans():
            # Group by worksheet and delete in reverse row order
            by_ws = {}
            for o in orphaned:
                key = o["username"]
                if key not in by_ws:
                    by_ws[key] = {"ws": o["ws"], "rows": []}
                by_ws[key]["rows"].append((o["row"], o["desc"]))
            for uname, data in by_ws.items():
                for row_num, desc in sorted(data["rows"], reverse=True):
                    data["ws"].delete_rows(row_num)
                    deleted.append(f"{desc} (@{uname})")

        await loop.run_in_executor(None, lambda: _sheets_call(_delete_orphans))

    # Build response
    lines = ["**🔍 Sheet Sync Audit**\n"]
    if deleted:
        lines.append(f"🗑️ **Removed {len(deleted)} orphaned row(s) from sheet:**")
        lines.extend(f"• {d}" for d in deleted[:15])
        if len(deleted) > 15:
            lines.append(f"  ... and {len(deleted)-15} more")
    if missing:
        lines.append(f"\n⚠️ **{len(missing)} DB bet(s) missing from sheet** (re-run backfill to restore):")
        lines.extend(f"• {m}" for m in missing[:10])
        if len(missing) > 10:
            lines.append(f"  ... and {len(missing)-10} more")
    if not deleted and not missing:
        lines.append("✅ DB and sheet are in sync — no orphaned rows found.")
