...
def _single_book_price(game, market_key, pick, books=("draftkings", "fanduel")):
    ...
    async def _run():
        ...
        for (sk, day), day_bets in sorted(by_day.items()):
            if credits["used"] >= max_credits:
                break
            ev_resp, _ = await loop.run_in_executor(None, _hget,
            ...
            for b in day_bets:
                ...
                if len(cands) == 0:
                    failed += 1; reasons["no_candidate"] += 1; continue
                if len(cands) > 1:
                    failed += 1; reasons["ambiguous"] += 1; continue
                ev = cands[0]
                eid = ev["id"]
                ...
                if btype == "moneyline" and not both:
                    pick = ("h2h", ev["home_team"] if hf in desc else ev["away_team"])
                elif btype == "total":
                    m = _TOTAL_RE.search(desc)
                    if m:
                    ...
                    if m:
                        pick = ("spreads", (ev["home_team"] if hf in desc else ev["away_team"],
                                            float(m.group(1))))
                if pick is None:
