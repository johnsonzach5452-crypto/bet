...
_sheets_client = None
# Persisted (DB-backed) tracker for "has this user's calendar been rebuilt
# this month" — replaces a plain in-memory dict that reset on every bot
# restart. That reset was the real bug: after ANY deploy, the next grade of
# ANY bet saw the tracker empty and rebuilt EVERY user's calendar tab from
# scratch (~15-20 real Sheets API calls), all buried inside what the rate
# limiter only counted as a single throttled call — invisible to the
# limiter, and exactly why "just grading one bet" could blow the quota.
async def _calendar_needs_rebuild(username):
    now_ct = datetime.now(CENTRAL)
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("""CREATE TABLE IF NOT EXISTS calendar_built (
            username TEXT PRIMARY KEY, year INTEGER, month INTEGER)""")
        await db.commit()
        cur = await db.execute(
            "SELECT year, month FROM calendar_built WHERE username=?", (username,))
        row = await cur.fetchone()
    return row is None or (row[0], row[1]) != (now_ct.year, now_ct.month)


async def _mark_calendar_built(username):
    now_ct = datetime.now(CENTRAL)
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "INSERT OR REPLACE INTO calendar_built (username, year, month) VALUES (?,?,?)",
            (username, now_ct.year, now_ct.month))
        await db.commit()
...
def _apply_data_sheet_formatting(ss, ws):
    ...
    # ── Column widths ───────────────────────────────────────────────────
    widths = [140,140,100,80,300,100,70,110,90,130,55,70,80,80,75,85,70,120,70,160,80,90,75,130,220]
...
_dashboard_setup_cache = set()   # usernames confirmed to have a dashboard tab

# Sliding-window rate limiter shared by every Sheets write in the bot.
# A fixed minimum gap between calls (what this used to be) only stops two
# calls from landing at the EXACT same instant — it does nothing to cap
# the total number of calls over a rolling minute, which is what actually
# trips Google's quota when someone logs many bets back to back to back.
# This tracks real call timestamps in a rolling window and makes a call
# WAIT for a free slot rather than firing and hoping the retry saves it.
# Google's real limit is 60 write requests/min per identity. The bot has a
# SINGLE service-account identity, so every write to every sheet shares one
# bucket. Default 55 leaves a little headroom.
_SHEETS_RATE_LIMIT = int(os.environ.get("SHEETS_RATE_LIMIT_PER_MIN", "55"))
_SHEETS_RATE_WINDOW = 60.0
_sheets_call_times = deque()
_sheets_throttle_lock = asyncio.Lock()

# CRITICAL CALIBRATION: one "call" here is a closure that may issue SEVERAL
# real Google requests internally (open sheet, find row, read a cell, batch
# write, verify...). Counting each closure as 1 badly under-counted actual
# usage — a grade is ~7 requests, and ~12 when it also rebuilds a calendar.
# At 50 closures/min that was really ~350 requests/min against a 60 cap,
# which is why limits still got hit. Callers now declare their true cost.
_SHEETS_COST = {
    "write":   4,    # open + get/create sheet + append + (dashboard check)
    "update":  5,    # open + sheet + find row + read stake + batch write
    "verify":  2,    # open + read one cell
    "delete":  2,
    "calendar": 5,   # clear/create + write grid + formatting batch
    "bulk":    10,   # full-sheet rewrite (/resync)
}


# Background loops (odds_watch, score_watch) write per pending bet, so a
# single cycle with many pending bets can demand far more slots than the
# window holds. Without prioritisation those would occupy the queue for
# minutes and a grade you just made would sit behind them. Interactive
# work therefore reserves ahead of background work.
_sheets_priority_waiters = 0


async def _call_sheets(fn, cost=1, background=False):
    """Run a Sheets function on the executor, reserving `cost` slots in the
    rolling window first. `cost` should reflect how many real Google
    requests the closure actually makes (see _SHEETS_COST), otherwise the
    limiter silently permits far more traffic than Google allows.

    background=True marks low-priority work (periodic polling). It yields
    to any interactive write that's waiting, so grading a bet never sits
    behind a long odds-refresh cycle."""
    global _sheets_priority_waiters
    loop = asyncio.get_event_loop()
    cost = max(1, min(int(cost), _SHEETS_RATE_LIMIT))
    if not background:
        _sheets_priority_waiters += 1
    try:
        return await _reserve_and_run(fn, cost, background, loop)
    finally:
        if not background:
            _sheets_priority_waiters -= 1


async def _reserve_and_run(fn, cost, background, loop):
    # Background work yields to pending interactive writes BEFORE taking the
    # lock. Doing this check inside the lock would mean sleeping while
    # holding it — the interactive caller could never acquire the lock to
    # make progress, so the yield could never end. Structured this way the
    # lock is only ever held for non-blocking bookkeeping.
    if background:
        for _ in range(40):          # ~10s ceiling, then proceed anyway
            if _sheets_priority_waiters == 0:
                break
            await asyncio.sleep(0.25)

    async with _sheets_throttle_lock:
        while True:
            now = loop.time()
            while _sheets_call_times and now - _sheets_call_times[0] > _SHEETS_RATE_WINDOW:
                _sheets_call_times.popleft()
            if len(_sheets_call_times) + cost <= _SHEETS_RATE_LIMIT:
                for _ in range(cost):
                    _sheets_call_times.append(now)
                break
            # not enough room — wait until the oldest calls age out
            sleep_for = _SHEETS_RATE_WINDOW - (now - _sheets_call_times[0]) + 0.05
            log.info(f"Sheets rate window full ({len(_sheets_call_times)}/"
                    f"{_SHEETS_RATE_LIMIT}, need {cost}) — "
                    f"queuing for {sleep_for:.1f}s")
            await asyncio.sleep(max(sleep_for, 0.1))
    return await loop.run_in_executor(None, lambda: _sheets_call(fn))


def _sheets_call(fn, tries=4):
    """Run a sheets function with resilience:
      - 429 / quota / rate-limit errors  -> exponential backoff and retry
      - auth / token / session errors     -> reset client and retry fresh
    Raises the last exception if all tries fail, so callers can report it."""
    global _sheets_client
    ...
    for attempt in range(tries):
        try:
            ...
            is_auth = ("401" in msg or "403" in msg or "token" in msg
                       or "unauthorized" in msg or "invalid_grant" in msg
                       or "credentials" in msg)
            if attempt == tries - 1:
                break
            if is_rate:
                wait = min(2 ** attempt, 8) + 0.5
                log.warning(f"Sheets rate-limited ({e}); backing off {wait}s "
                            f"(attempt {attempt+1}/{tries})")
                _time.sleep(wait)
            elif is_auth:
                log.warning(f"Sheets auth error ({e}); resetting client")
                _sheets_client = None
                _time.sleep(0.5)
            else:
                log.warning(f"Sheets call failed ({e}); retrying")
                _time.sleep(0.5)
    raise last
...
def _get_or_create_data_sheet(ss, username):
    ...
    Also ensures an EXISTING sheet is wide enough for all columns. Sheets
    created before a new column was added (e.g. Player Img -> col Y/25) keep
    their original width, so any write touching the new column fails with
    'exceeds grid limits'. Widen the grid here — the single chokepoint every
    sheet write passes through — so the fix applies everywhere at once."""
    need_cols = len(SHEET_HEADERS)
    try:
        ws = ss.worksheet(username)
        if (ws.col_count or 0) < need_cols:
            ws.add_cols(need_cols - ws.col_count)
            log.info(f"widened '{username}' sheet to {need_cols} columns")
            # make sure the header row covers the new columns too
            try:
                ws.update([SHEET_HEADERS], "A1", value_input_option="RAW")
            except Exception:
                log.exception("header refresh after widen failed")
    except gspread.WorksheetNotFound:
        ws = ss.add_worksheet(title=username, rows=2000, cols=need_cols)
...
# ═══════════════════════════════════════════════════════════════════════
# PROP CLV ENGINE — closing-line tracking for player props
# Player → game resolution happens at log time (event odds cached per day),
# closing capture happens in the same pre-game window as game bets.
# Strict everywhere: unknown market, ambiguous player, or missing line = skip.
# ═══════════════════════════════════════════════════════════════════════
_PROP_MARKETS = {
    "baseball_mlb": [
        (("strikeout", "k's", " ks"), "pitcher_strikeouts"),
        (("total base",), "batter_total_bases"),
        (("home run", "hr"), "batter_home_runs"),
        (("hit",), "batter_hits"),
        (("rbi",), "batter_rbis"),
        (("earned run",), "pitcher_earned_runs"),
        (("out", "pitching out"), "pitcher_outs"),
    ],
    "basketball_wnba": [
        (("pra", "pts+reb+ast", "points rebounds assists"), "player_points_rebounds_assists"),
        (("point", "pts"), "player_points"),
        (("assist", "ast"), "player_assists"),
        (("rebound", "reb"), "player_rebounds"),
        (("three", "3pt", "3-pt"), "player_threes"),
    ],
}
_PROP_MARKETS["basketball_nba"] = _PROP_MARKETS["basketball_wnba"]

_EVENT_ODDS_CACHE = {}   # (sk, event_id, market) -> (fetched_at_ts, data)
_EVENT_ODDS_TTL = 480    # 8 minutes


def _prop_market_key(sk, prop_category, desc):
    """Resolve the API market key from prop_category or description text."""
    hints = ((prop_category or "") + " " + (desc or "")).lower()
    for keywords, market in _PROP_MARKETS.get(sk, []):
        if any(k in hints for k in keywords):
            return market
    return None


def _parse_prop(desc):
    """'Paul Skenes Over 7.5 Strikeouts' -> ('paul skenes...', 'Over', 7.5).
    Player text is everything before the over/under token (loose — matching
    against outcome names is done word-wise, so extra words are fine)."""
    m = _TOTAL_RE.search(desc or "")
    if not m:
        return None, None, None
    player_text = (desc or "")[:m.start()].lower()
    player_text = re.sub(r"[^a-z.\s'-]", " ", player_text)
    return player_text.strip(), m.group(1).capitalize(), float(m.group(2))


def _player_matches(outcome_name, player_text):
    """Strict: outcome's last name (word-boundary) AND first name or first
    initial must appear in the bet's player text."""
    parts = (outcome_name or "").lower().replace(".", "").split()
    if not parts:
        return False
    last = parts[-1]
    if not re.search(rf"\b{re.escape(last)}\b", player_text.replace(".", "")):
        return False
    if len(parts) == 1:
        return True
    first = parts[0]
    pt = player_text.replace(".", "")
    return bool(re.search(rf"\b{re.escape(first)}\b", pt)
                or re.search(rf"\b{re.escape(first[0])}\b", pt))


def _fetch_events(sk):
    """Quota-free upcoming/in-play events list."""
    import requests as _rq
    try:
        r = _rq.get(f"https://api.the-odds-api.com/v4/sports/{sk}/events",
...
def _fetch_event_prop_odds(sk, event_id, market):
    """Event-odds call for one prop market, cached briefly."""
    import time as _time, requests as _rq
    key = (sk, event_id, market)
    hit = _EVENT_ODDS_CACHE.get(key)
    if hit and _time.time() - hit[0] < _EVENT_ODDS_TTL:
        return hit[1]
    try:
        r = _rq.get(
            f"https://api.the-odds-api.com/v4/sports/{sk}/events/{event_id}/odds",
            params={"apiKey": ODDS_API_KEY, "regions": "us",
                    "markets": market, "oddsFormat": "american"}, timeout=15)
        data = r.json() if r.status_code == 200 else None
    except Exception:
        data = None
    _EVENT_ODDS_CACHE[key] = (_time.time(), data)
    return data


def _prop_price(data, market, player_text, side, point):
    """Consensus (2+ books) for a player prop line; single trusted book as
    fallback. Ambiguous player match anywhere -> None (strict)."""
    ...
    for bk in data.get("bookmakers", []):
        for mkt in bk.get("markets", []):
            if mkt.get("key") != market:
                continue
            hits = [oc for oc in mkt.get("outcomes", [])
                    if oc.get("name") == side and oc.get("point") == point
                    and _player_matches(oc.get("description", ""), player_text)]
            names = {oc.get("description") for oc in hits}
            if len(names) > 1:
                return None    # two different players matched -> ambiguous
            if hits:
                probs.append(_implied(hits[0]["price"]))
                single[bk.get("key")] = hits[0]["price"]
    if len(probs) >= 2:
        return _prob_to_american(sum(probs) / len(probs))
    for bk in ("draftkings", "fanduel", "betmgm", "caesars",
               "pointsbetus", "bovada", "betrivers"):
        if bk in single:
            return _prob_to_american(_implied(single[bk]))
    return None


async def _resolve_prop_event(sk, market, player_text, max_tries=5):
    """Find which event this player belongs to by probing event prop odds,
    nearest games first. Returns (event_id, commence, data) or (None,)*3."""
    loop = asyncio.get_event_loop()
    events = await loop.run_in_executor(None, _fetch_events, sk)
    now = datetime.now(timezone.utc)
    def _dist(e):
        try:
            ct = datetime.fromisoformat(e["commence_time"].replace("Z", "+00:00"))
            return abs((ct - now).total_seconds())
        except Exception:
            return float("inf")
    events = [e for e in events if _dist(e) < 40 * 3600]
    events.sort(key=_dist)
    tries = 0
    for e in events:
        cached = _EVENT_ODDS_CACHE.get((sk, e["id"], market))
        fresh = cached and (__import__("time").time() - cached[0] < _EVENT_ODDS_TTL)
        if not fresh:
            if tries >= max_tries:
                break
            tries += 1
        data = await loop.run_in_executor(
            None, _fetch_event_prop_odds, sk, e["id"], market)
        if not data:
            continue
        for bk in (data.get("bookmakers") or []):
            for mkt in bk.get("markets", []):
                for oc in mkt.get("outcomes", []):
                    if _player_matches(oc.get("description", ""), player_text):
                        return e["id"], e.get("commence_time"), data
    return None, None, None


async def _market_check(data, bet_msg, username=""):
    """Right after a bet is logged, compare the entry price against current
    market consensus and reply with the verdict — feedback at the one moment
    it's still actionable. Also persists game_time immediately so the Live
    section doesn't have to wait for the next odds poll. Game markets only;
    silent when no clean match (props handled by the prop engine)."""
    try:
        btype = (data.get("bet_type") or "").lower()
        if data.get("odds") is None:
            return
        sk = _odds_sport_key(data.get("sport"), data.get("league"))
        if not sk or not ODDS_API_KEY:
            return

        if btype == "prop":
            # ── prop path: find the player's game, store it, check price ──
            desc = data.get("description") or ""
            market = _prop_market_key(sk, data.get("prop_category"), desc)
            player, side, point = _parse_prop(desc)
            if not market or not player or point is None:
                return
            eid, commence, pdata = await _resolve_prop_event(sk, market, player)
            if not eid:
                return
            async with aiosqlite.connect(DB_PATH) as db:
                await db.execute(
                    "UPDATE bets SET event_id=?, game_time=? WHERE message_id=?",
                    (eid, commence, str(bet_msg.id)))
                await db.commit()
            # headshot for the dashboard (bot cards resolve it at render time)
            _img = _headshot_url(sk, player)
            if _img:
                async with aiosqlite.connect(DB_PATH) as db:
                    await db.execute("UPDATE bets SET player_img=? WHERE message_id=?",
                                     (_img, str(bet_msg.id)))
                    await db.commit()
            await sheets_update_odds(username, str(bet_msg.id), game_time=commence,
                                     player_img=_img)
            price = _prop_price(pdata, market, player, side, point)
            if price is None:
                return
            entry = _to_american(data["odds"])
            diff = round((_implied(price) - _implied(entry)) * 100, 1)
            e_str, p_str = f"{entry:+.0f}", f"{price:+.0f}"
            if diff >= 0.5:
                verdict = f"beating market by **{diff:.1f} pts** ✅"
            elif diff <= -0.5:
                verdict = f"paying **{abs(diff):.1f} pts** over market"
            else:
                verdict = "right at market"
            await bet_msg.reply(
                f"📊 Market check: you got **{e_str}** · consensus **{p_str}** → {verdict}",
                mention_author=False)
            return

        if btype not in ("moneyline", "total", "spread"):
            return
        games = await asyncio.get_event_loop().run_in_executor(
            None, _fetch_sport_odds, sk)
        if not games:
            return
        price, commence = _match_bet_price(
            {"description": data.get("description"),
             "bet_type": btype, "odds": data.get("odds")}, games)
        if commence:
            async with aiosqlite.connect(DB_PATH) as db:
                await db.execute("UPDATE bets SET game_time=? WHERE message_id=?",
                                 (commence, str(bet_msg.id)))
                await db.commit()
            # sheet too — the dashboard reads the sheet, and odds_watch will
            # skip its own write once the DB already matches
            await sheets_update_odds(username, str(bet_msg.id), game_time=commence)
        if price is None:
            return
        entry = _to_american(data["odds"])
        diff = round((_implied(price) - _implied(entry)) * 100, 1)
        e_str = f"{entry:+d}" if isinstance(entry, int) else f"{entry:+.0f}"
        p_str = f"{price:+d}" if isinstance(price, int) else f"{price:+.0f}"
        if diff >= 0.5:
            verdict = f"beating market by **{diff:.1f} pts** ✅"
        elif diff <= -0.5:
            verdict = f"paying **{abs(diff):.1f} pts** over market"
        ...
        log.exception("market check failed")
...
async def sheets_log_bet(bet_data, username, group_name, message_id):
    ...
    try:
        def _write():
            ...
            # Manual /logbet backdating (e.g. "log yesterday's bet") sets
            # _manual_placed_at — use it for the sheet's Date Posted column
            # so the calendar (which reads this column) puts the bet on the
            # right day. Normal auto-parsed bets never set this, so they
            # fall through to "now" exactly as before.
            _placed = bet_data.get("_manual_placed_at") or datetime.now(CENTRAL)
            row = [
                _placed.strftime("%Y-%m-%d %I:%M %p"),  # A Date Posted
            ...
            # Create/refresh dashboard and calendar on first use only.
            # ss.worksheets() is a full extra API call — cache which users
            # already have their dashboard set up so this only runs ONCE
            # per user ever, not on every single bet log (this was doubling
            # the Sheets API calls per bet during normal use).
            if username not in _dashboard_setup_cache:
                ...
                _dashboard_setup_cache.add(username)
        await _call_sheets(_write, cost=_SHEETS_COST["write"])
    except Exception as e:
        log.exception("sheets_log_bet failed")
        emsg = str(e).lower()
        if "429" in emsg or "quota" in emsg or "rate" in emsg or "exhausted" in emsg:
            reason = ("Google Sheets API rate limit hit while logging this bet. "
                      "It's tracked internally — run /resync once quota resets "
                      "(usually under a minute) to push it to the sheet.")
        elif ("401" in emsg or "403" in emsg or "token" in emsg
              or "credentials" in emsg or "invalid_grant" in emsg):
            reason = ("Google auth failed while logging this bet — the service-account "
                      "credentials may be expired or the sheet un-shared. Fix auth then /resync.")
        else:
            reason = (f"Unexpected sheet error: {type(e).__name__}: {str(e)[:180]}. "
                      f"Run /resync to retry once resolved.")
        await post_monitor("Sheets Write Failed", reason, level="error")



async def _db_streak(db, user_id, group_name):
    """Current W/L streak string from the DB, ordered by settlement time."""
    cur = await db.execute(
    ...
    for s in reversed(rows):
        if s == last:
            ...
            break
    return f"{'W' if last == 'won' else 'L'}{count}"
...
async def sheets_update_bet(username, message_id, status, profit, cumulative_pnl=None,
    ...
    cumulative_pnl and streak_str should be passed from the DB by the caller —
    computing them here would require reading full sheet columns on every
    grade, which is the main driver of Sheets API rate-limit failures."""
    client = _get_sheets_client()
    if client is None:
        return
    loop = asyncio.get_event_loop()
    # Checked ONCE here (async, outside the throttled write) rather than
    # with an in-memory flag that reset every deploy — see _calendar_needs_rebuild.
    need_cal_rebuild = await _calendar_needs_rebuild(username)
    try:
        def _update():
            ss = client.open_by_key(SPREADSHEET_ID)
            ws = _get_or_create_data_sheet(ss, username)
            ...
            # Streak — prefer the DB-computed value the caller passed in.
            # Only fall back to reading the sheet column if it wasn't supplied
            # (keeps grade-time API calls minimal to avoid rate limits).
            nonlocal streak_str
            if streak_str is None:
            ...
            # Rebuild ONLY this user's calendar tab, only when actually needed
            # (checked once via persisted DB state above) — not every user's
            # tab on every grade, which was the real driver of hidden call
            # spikes disguised as "one grade" hitting the rate limit.
            if need_cal_rebuild:
                try:
                    _create_calendar_tab(ss, username)
                except Exception:
                    log.exception("calendar auto-rebuild failed")
        # cost includes the calendar rebuild when that will run inside it
        await _call_sheets(_update, cost=_SHEETS_COST["update"] +
                           (_SHEETS_COST["calendar"] if need_cal_rebuild else 0))
        if need_cal_rebuild:
            await _mark_calendar_built(username)
        ...
        ok = await _call_sheets(_verify, cost=_SHEETS_COST["verify"])
        if ok is False:
            log.warning(f"sheet grade for {message_id} didn't stick — retrying once")
            await _call_sheets(_update, cost=_SHEETS_COST["update"])
            ok2 = await _call_sheets(_verify, cost=_SHEETS_COST["verify"])
            if ok2 is False:
        ...
        emsg = str(e).lower()
        if "429" in emsg or "quota" in emsg or "rate" in emsg or "exhausted" in emsg:
            reason = ("Google Sheets API rate limit hit. The grade is saved in the bot "
                      "database — run /resync to push it to the sheet once quota resets "
                      "(usually under a minute).")
        elif ("401" in emsg or "403" in emsg or "token" in emsg
              or "credentials" in emsg or "invalid_grant" in emsg):
            reason = ("Google auth failed — the service-account credentials may be expired "
                      "or the sheet un-shared with the service account. Grade is in the DB; "
                      "fix auth then /resync.")
        else:
            reason = (f"Unexpected sheet error: {type(e).__name__}: {str(e)[:180]}. "
                      f"Grade is saved in the DB — run /resync to retry.")
        await post_monitor("Sheets Update Failed", reason, level="error")
...
async def sheets_delete_bet(username, message_id):
    ...
    try:
        ...
        await _call_sheets(_delete, cost=_SHEETS_COST["delete"])
    except Exception:
...
def _odds_from_payout(stake, potential_payout):
    """Derive American odds directly from stake and total payout — immune to
    any odds-FORMAT confusion (American vs percent vs decimal). Novig has
    started displaying some slips as a percentage instead of American odds,
    and the screenshot parse doesn't always catch the switch; payout math
    sidesteps the whole problem since it's pure arithmetic on two dollar
    amounts the slip always shows clearly. Returns None when the inputs
    don't support a sane calculation (missing, zero stake, or a payout that
    implies no profit) — caller falls back to the parsed odds in that case."""
    try:
        stake = float(stake)
        payout = float(potential_payout)
    except (TypeError, ValueError):
        return None
    if stake <= 0 or payout <= stake:
        return None
    decimal_odds = payout / stake
    if decimal_odds >= 2.0:
...
# ═══════════════════════════════════════════════════════════════════════
# ESPN PLAYER HEADSHOTS — bulk ID index, cached locally
# One API call per league per refresh pulls ESPN's entire athlete list
# (name + id + headshot href). It's cached in SQLite AND held in memory,
# so a prop bet's headshot lookup is a dict hit — no per-bet API calls.
# Strict by design: an ambiguous name resolves to nothing rather than
# risking the wrong player's face next to a bet.
# ═══════════════════════════════════════════════════════════════════════
_ESPN_LEAGUE_PATH = {
...
_HEADSHOT_SLUG = {"baseball_mlb": "mlb", "basketball_nba": "nba",
                  "basketball_wnba": "wnba"}
# in-memory: sport_key -> { last_name: [ {first, id, href, full} ] }
_PLAYER_INDEX = {}
_PLAYER_INDEX_TTL_DAYS = 7


def _norm_name(s):
    """Lowercase, strip punctuation/accents-lite, collapse whitespace."""
    s = (s or "").lower().replace(".", "").replace("'", "").replace("-", " ")
    s = re.sub(r"[^a-z\s]", " ", s)
    return re.sub(r"\s+", " ", s).strip()


def _fetch_espn_athletes(sk):
    """Pull ESPN's full athlete list for a league. Returns list of dicts."""
    path = _ESPN_LEAGUE_PATH.get(sk)
    if not path:
        return []
    import requests as _rq
    url = (f"https://sports.core.api.espn.com/v3/sports/{path[0]}/{path[1]}"
           f"/athletes?limit=20000&active=true")
    try:
        r = _rq.get(url, timeout=30)
        if r.status_code != 200:
            log.warning(f"espn athletes {sk} -> {r.status_code}")
            return []
        j = r.json()
    except Exception:
        log.exception(f"espn athlete fetch failed for {sk}")
        return []
    # v3 returns "items"; be tolerant of alternate shapes
    raw = j.get("items") or j.get("athletes") or []
    out = []
    for a in raw:
        ...
        if not pid or not (last or full):
            continue
        if not last and full:
            parts = full.split()
            first, last = (parts[0], parts[-1]) if len(parts) > 1 else ("", full)
        href = ""
        hs = a.get("headshot")
        if isinstance(hs, dict):
            href = hs.get("href") or ""
        out.append({"id": pid, "full": full or f"{first} {last}".strip(),
                    "first": _norm_name(first), "last": _norm_name(last),
                    "href": href})
    return out


async def refresh_player_index(sk, force=False):
    """Refresh one league's athlete index if stale. Returns rows stored."""
    if sk not in _ESPN_LEAGUE_PATH:
        return 0
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("""CREATE TABLE IF NOT EXISTS espn_players (
            sport TEXT, espn_id TEXT, full_name TEXT, first TEXT, last TEXT,
            href TEXT, PRIMARY KEY (sport, espn_id))""")
        await db.execute("""CREATE TABLE IF NOT EXISTS espn_index_meta (
            sport TEXT PRIMARY KEY, updated_at TEXT)""")
        await db.execute(
            "CREATE INDEX IF NOT EXISTS idx_espn_last ON espn_players(sport, last)")
        await db.commit()
        if not force:
            cur = await db.execute(
                "SELECT updated_at FROM espn_index_meta WHERE sport=?", (sk,))
            row = await cur.fetchone()
            if row and row[0]:
                try:
                    age = datetime.now(timezone.utc) - datetime.fromisoformat(row[0])
                    if age.days < _PLAYER_INDEX_TTL_DAYS:
                        await _load_player_index(sk)
                        return -1     # still fresh; loaded from cache
                except ValueError:
                    pass

    players = await asyncio.get_event_loop().run_in_executor(
        None, _fetch_espn_athletes, sk)
    if not players:
        await _load_player_index(sk)   # keep whatever we already had
        return 0
    async with aiosqlite.connect(DB_PATH) as db:
        await db.executemany(
            "INSERT OR REPLACE INTO espn_players "
            "(sport, espn_id, full_name, first, last, href) VALUES (?,?,?,?,?,?)",
            [(sk, p["id"], p["full"], p["first"], p["last"], p["href"])
             for p in players])
        await db.execute(
            "INSERT OR REPLACE INTO espn_index_meta (sport, updated_at) VALUES (?,?)",
            (sk, datetime.now(timezone.utc).isoformat()))
        await db.commit()
    await _load_player_index(sk)
    log.info(f"player index refreshed for {sk}: {len(players)} athletes")
    return len(players)


async def _load_player_index(sk):
    """Load a league's athletes from SQLite into the in-memory index."""
    try:
        async with aiosqlite.connect(DB_PATH) as db:
            db.row_factory = aiosqlite.Row
            cur = await db.execute(
                "SELECT espn_id, full_name, first, last, href FROM espn_players "
                "WHERE sport=?", (sk,))
    ...
    idx = {}
    for r in rows:
        idx.setdefault(r["last"], []).append(
            {"id": r["espn_id"], "full": r["full_name"],
             "first": r["first"], "href": r["href"]})
    _PLAYER_INDEX[sk] = idx


def _lookup_player(sk, player_text):
    """Resolve a bet's player text to one athlete. STRICT — returns the
    athlete dict only on an unambiguous match, else None.

    'Paul Skenes'   -> matches (last + first)
    'P. Skenes'     -> matches (last + first initial)
    'Skenes'        -> matches only if exactly one Skenes exists
    'Castillo'      -> None when several Castillos exist (never guesses)"""
    idx = _PLAYER_INDEX.get(sk)
    if not idx:
        return None
    text = _norm_name(player_text)
    if not text:
        return None
    tokens = text.split()
    hits = []
    for tok in tokens:
        for cand in idx.get(tok, []):
            first = cand.get("first") or ""
            # require the first name (or its initial) to also appear, unless
            # the surname is unique league-wide
            same_last = idx.get(tok, [])
            if len(same_last) == 1:
                hits.append(cand)
                continue
            if first and (re.search(rf"\b{re.escape(first)}\b", text)
                          or re.search(rf"\b{re.escape(first[0])}\b", text)):
                hits.append(cand)
...
def _headshot_url(sk, player_text):
    """Headshot URL for a bet's player, or None. Prefers ESPN's own href
    (guaranteed to exist) over a constructed URL."""
    p = _lookup_player(sk, player_text)
    if not p:
        return None
    if p.get("href"):
        return p["href"]
    slug = _HEADSHOT_SLUG.get(sk)
    if not slug or not p.get("id"):
        return None
    return f"https://a.espncdn.com/i/headshots/{slug}/players/full/{p['id']}.png"


@tasks.loop(hours=24)
async def player_index_watch():
    """Keep athlete indexes fresh (no-op when already current)."""
    for sk in _ESPN_LEAGUE_PATH:
        try:
            await refresh_player_index(sk)
        except Exception:
            log.exception(f"player index refresh failed for {sk}")


@player_index_watch.before_loop
async def before_player_index_watch():
    await bot.wait_until_ready()


# ── ESPN team logos for Discord bet cards ───────────────────────────────
# nickname -> (espn league slug, espn team code). Nicknames are unique
# across MLB/NBA/WNBA, so a nickname alone resolves the logo.
_ESPN_LOGO = {
    # MLB
    "diamondbacks": ("mlb","ari"), "braves": ("mlb","atl"), "orioles": ("mlb","bal"),
    ...
    "yankees": ("mlb","nyy"), "athletics": ("mlb","oak"), "phillies": ("mlb","phi"),
    "pirates": ("mlb","pit"), "padres": ("mlb","sd"), "mariners": ("mlb","sea"),
    "giants": ("mlb","sf"), "cardinals": ("mlb","stl"), "rays": ("mlb","tb"),
    "rangers": ("mlb","tex"), "blue jays": ("mlb","tor"), "jays": ("mlb","tor"),
    "nationals": ("mlb","wsh"),
    # NBA
    ...
    "magic": ("nba","orl"), "76ers": ("nba","phi"), "sixers": ("nba","phi"),
    "suns": ("nba","phx"), "trail blazers": ("nba","por"), "blazers": ("nba","por"),
    "kings": ("nba","sac"), "spurs": ("nba","sa"), "raptors": ("nba","tor"),
    "jazz": ("nba","utah"), "wizards": ("nba","wsh"),
    # WNBA
    "dream": ("wnba","atl"), "sky": ("wnba","chi"), "sun": ("wnba","conn"),
    "wings": ("wnba","dal"), "fever": ("wnba","ind"), "aces": ("wnba","lv"),
    "sparks": ("wnba","la"), "lynx": ("wnba","min"), "liberty": ("wnba","ny"),
    "mercury": ("wnba","phx"), "storm": ("wnba","sea"), "mystics": ("wnba","wsh"),
    "valkyries": ("wnba","gs"), "tempo": ("wnba","tor"),
}
def _espn_allowed_slugs(sport, league):
    """Which ESPN leagues may this bet's logo come from? Scoping is what
    prevents an MLB bet from ever resolving to an NBA/WNBA crest."""
    ...
    if "baseball" in sp:
        return {"mlb"}
    if "basketball" in sp:
        return {"nba", "wnba"}      # league unknown
    return None                      # unknown -> allow all


def _espn_logo_url(description, sport=None, league=None):
    """Logo for the team the bet is ON — the one named FIRST, matching the
    side-detection rule ('Fever -9.5 vs Storm' -> Fever, not Storm).

    Word-boundary matched so "Sunday" can't match the Sun and "Hornets"
    can't match the Nets, and league-scoped so a baseball bet never picks
    up a basketball logo. Returns a URL or None (None = no thumbnail)."""
    raw = description or ""
    d = raw.lower()
    allow = _espn_allowed_slugs(sport, league)
    best, best_pos = None, len(d) + 1
    for nick, ref in _ESPN_LOGO.items():
        if allow is not None and ref[0] not in allow:
            continue
        m = re.search(rf"\b{re.escape(nick)}\b", d)
        if m and m.start() < best_pos:
            best, best_pos = ref, m.start()
    if not best and allow == {"mlb"}:
        # abbreviations, only when the league is unambiguous
        for abbr, frag in _MLB_ABBR_TO_FRAG.items():
            if re.search(rf"\b{abbr}\b", raw) and frag in _ESPN_LOGO:
                best = _ESPN_LOGO[frag]
                break
    if not best:
        return None
    return f"https://a.espncdn.com/i/teamlogos/{best[0]}/500/{best[1]}.png"
...
def build_embed(data, author_name, status="pending", profit=None, record=None):
    ...
    # Thumbnail: a player prop shows that player's headshot when the name
    # resolves unambiguously; otherwise (and for all game bets) the team
    # logo. Never guesses — an unresolved name simply gets the team crest.
    try:
        _thumb = None
        if (bt or "").lower() == "prop":
            _sk = _odds_sport_key(sport, league)
            _player, _side, _pt = _parse_prop(data.get("description"))
            if _sk and _player:
                _thumb = _headshot_url(_sk, _player)
        if not _thumb:
            _thumb = _espn_logo_url(data.get("description"), sport, league)
        if _thumb:
...
async def on_ready():
    ...
    if not player_index_watch.is_running():
        player_index_watch.start()
        log.info("player_index_watch started (ESPN headshot index, daily)")
    if not nightly_recap.is_running():
        nightly_recap.start()
        log.info("nightly_recap started (2am ET)")
    if ODDS_API_KEY and not score_watch.is_running():
        ...
        log.info("score_watch started ("
                 ...
                 + ", misgrade detection only — auto-grading removed)")
...
async def on_message(message):
    ...
    try:
        ...
        # Novig sometimes shows odds as a percentage instead of American
        # odds, and the vision parse doesn't always catch the format switch.
        # Payout math is immune to that ambiguity — whenever both stake and
        # the total payout are available, derive the odds directly from
        # them and trust that over whatever format the parse assumed.
        # Falls back to normalizing the parsed value (handles a bare
        # percentage/decimal) when payout math isn't available.
        derived_odds = _odds_from_payout(data.get("stake"), data.get("potential_payout"))
        if derived_odds is not None:
            data["odds"] = derived_odds
        elif data.get("odds") is not None:
            try:
                data["odds"] = _to_american(data["odds"])
            except (TypeError, ValueError):
                pass
    except Exception:
    ...
    if _lg_note:
        log.info(f"silent correction: {_lg_note}")   # applied, not announced
    ...
    bot.loop.create_task(_market_check(data, bet_msg, message.author.display_name))
...
async def _handle_reaction(reaction, user):
    ...
    if msg_id in _awaiting_confirmation:
        ...
        if emoji == CONFIRM_EMOJI:
            ...
            bot.loop.create_task(_market_check(data, reaction.message, pending["username"]))
            await refresh_live_stats(group_name)
            return
    ...
    # Cumulative P&L and current streak, both from the DB (authoritative and
    # avoids reading full sheet columns on every grade — the rate-limit fix)
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        streak_str = await _db_streak(db, bet["user_id"], group_name)

    await sheets_update_bet(bet["username"], reaction.message.id, status, profit,
                            cumulative_pnl=round(cumulative_pnl, 2), streak_str=streak_str)
    await refresh_live_stats(group_name)
...
def _parse_manual_date(s):
    """Parse a casual date string ('yesterday', '8/30', '2026-08-30') for
    /logbet's optional backdate option. Returns a CENTRAL-timezone datetime
    at noon (avoids landing on the wrong day near a midnight boundary), or
    None if the text isn't recognized — caller decides what to do then."""
    ...
    if s in ("today",):
        ...
        for fmt in ("%Y-%m-%d", "%m/%d/%Y", "%m/%d/%y", "%m/%d",
            ...
            try:
                ...
                year = parsed.year if ("%Y" in fmt or "%y" in fmt) else now_ct.year
                d = datetime(year, parsed.month, parsed.day).date()
                break
            except ValueError:
                continue
        if d is None:
            return None
    return datetime(d.year, d.month, d.day, 12, 0, 0, tzinfo=CENTRAL)
...
@app_commands.describe(
    ...
    date_placed="When you placed it, e.g. 'yesterday', '8/30', '2026-08-30' (optional, defaults to now)",
)
...
async def logbet(
    ...
    manual_dt = _parse_manual_date(date_placed) if date_placed else None
    date_warning = (date_placed and manual_dt is None)  # they typed something, couldn't parse it
    ...
    if output_channel_id:
        ...
        confirm = f"Logged — see {target_channel.mention}"
        if date_warning:
            confirm += (f"\n⚠️ Couldn't understand the date '{date_placed}' — logged with "
                       f"today's date instead. Try 'yesterday' or 'YYYY-MM-DD'.")
        ...
        if date_warning:
            await interaction.followup.send(
                f"⚠️ Couldn't understand the date '{date_placed}' — logged with today's date "
                f"instead. Try 'yesterday' or 'YYYY-MM-DD'.", ephemeral=True)
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            ...
            (
                ...
                stake,
                potential_payout,
                (manual_dt.astimezone(timezone.utc) if manual_dt
...
async def fixbet(
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        # Re-derive odds from the corrected stake/payout. Without this,
        # fixing a bet whose odds were parsed wrong (e.g. Novig showing a
        # percentage) would leave the stale odds in place, and CLV would
        # keep comparing against a number that was never right.
        new_odds = _odds_from_payout(stake, payout)
        if new_odds is not None:
            await db.execute(
                "UPDATE bets SET stake=?, potential_payout=?, profit=?, odds=? "
                "WHERE message_id=?",
                (stake, payout, profit, new_odds, message_id),
            )
            bet["odds"] = new_odds       # keep the rebuilt embed in sync
        else:
            await db.execute(
...
# Minimum implied-probability move (in points) before a live-odds change is
# written to the sheet. Filters out cosmetic ticks that would otherwise
# consume Sheets quota shared with grading. DB always records every change.
LIVE_ODDS_MIN_MOVE_PTS = float(os.environ.get("LIVE_ODDS_MIN_MOVE_PTS", "1.0"))
...
def _team_pos(team_name, desc_lower, desc_raw, before=None):
    """Rightmost position where this team is referenced (optionally only
    counting mentions BEFORE a given position), or -1. Checks nickname,
    MLB abbreviations, full city name, and city codes."""
    best = -1
    def consider(pos):
        nonlocal best
    ...
    if frag:
        for m in re.finditer(rf"\b{re.escape(frag)}\b", desc_lower):
            consider(m.start())
    for abbr, af in _MLB_ABBR_TO_FRAG.items():
        if af == frag:
            for m in re.finditer(rf"\b{abbr}\b", desc_raw):
                consider(m.start())
    city = " ".join((team_name or "").split(" ")[:-1]).lower()
    if city:
        for m in re.finditer(re.escape(city), desc_lower):
            consider(m.start())
        for abbr, c in _CITY_ABBR.items():
            if c == city:
                for m in re.finditer(rf"\b{abbr}\b", desc_raw):
                    consider(m.start())
    return best
...
def _side_before(desc_raw, home, away, anchor):
    """When both teams are named, the pick is the team mentioned closest
    BEFORE the anchor position (e.g. the spread number or 'ML' token).
    'Fever -9.5 vs Storm' -> Fever. Returns team name or None."""
    d = desc_raw.lower()
    hp = _team_pos(home, d, desc_raw, before=anchor)
    ap = _team_pos(away, d, desc_raw, before=anchor)
    if hp < 0 and ap < 0:
        return None
    return home if hp > ap else away


def _strict_candidates(desc_raw, games):
    """Return [game] only when exactly one game matches AND no other team
    referenced in the description conflicts with it.

    This is the fix for the Fever/Valkyries misgrade: a bet on
    "Fever -9.5 vs Seattle Storm" matched a Fever-vs-Valkyries game because
    only one 'fever' game existed in the feed and the old matcher never
    checked that the OTHER named team (Storm) belonged to the same game.
    Now: if the description references any team -- from the feed OR from the
    known team lists -- that isn't in the matched game, the match is rejected."""
    ...
    if len(cands) != 1:
        return []
    g = cands[0]
    gteams = {g.get("home_team"), g.get("away_team")} - {None}
    gfrags = {_team_frag(t) for t in gteams}

    # conflict check 1: any team from any OTHER game in the feed
    # (narrow reference test -- city-level mentions don't count as conflicts)
    for other in games:
        for t in (other.get("home_team"), other.get("away_team")):
            if t and t not in gteams and _team_conflict_ref(t, desc, desc_raw):
                return []
    # conflict check 2: any known team nickname (catches teams whose games
    # aren't in the current feed window at all)
    known = set(_NBA_TEAMS) | set(_WNBA_TEAMS) | set(_MLB_ABBR_TO_FRAG.values())
    for frag in known:
        if frag not in gfrags and re.search(rf"\b{frag}\b", desc):
            return []
    return [g]


def _match_bet_price(bet, games):
    """Find the bet's game+market in the odds feed.
    Returns (american_price_or_None, commence_iso_or_None).
    IMPORTANT: these two are independent. The game itself (and its start
    time) can be found via team-name matching even when no bookmaker is
    currently posting a price for it -- e.g. many books briefly suspend
    markets right at first pitch/tipoff. Losing commence_time in that
    window is exactly what breaks the Live section, since a game with a
    live-eligible commence_time but no price would otherwise be silently
    dropped as (None, None) and never get its game_time persisted."""
    ...
    cands = _strict_candidates(desc_raw, games)
    if not cands:
        return None, None
    g = cands[0]
    home, away = g.get("home_team", ""), g.get("away_team", "")
    commence = g.get("commence_time")  # always available once we have 1 candidate
    ...
    if btype == "moneyline":
        ...
        if not both_named:
            ...
            mm = re.search(r"\b(ml|moneyline)\b", desc)
            if mm:
                team = _side_before(desc_raw, home, away, mm.start())
        if team:
            price = _consensus_price(g, "h2h", team)
    elif btype == "total":
        m = _TOTAL_RE.search(desc)
        if m:
            side = m.group(1).capitalize()
            point = float(m.group(2))
            price = _consensus_price(g, "totals", (side, point))
    elif btype == "spread":
        ...
        if m:
            ...
            if not both_named:
                ...
                team = _side_before(desc_raw, home, away, m.start())
            if team:
                point = float(m.group(1))
                price = _consensus_price(g, "spreads", (team, point))
    return price, commence


async def sheets_update_odds(username, message_id, live=None, closing=None, clv=None,
                             game_time=None, player_img=None, background=False):
    """Write Live Odds (U/21), Closing Odds (V/22), CLV pts (W/23),
    Game Time (X/24), Player Img (Y/25)."""
    client = _get_sheets_client()
    if client is None:
        return
    loop = asyncio.get_event_loop()
    ...
    def _upd():
        ...
        if player_img is not None:
            cells.append({"range": gspread.utils.rowcol_to_a1(row, 25), "values": [[player_img]]})
        if cells:
            ws.batch_update(cells, value_input_option="USER_ENTERED")

    try:
        await _call_sheets(_upd, cost=_SHEETS_COST["update"], background=background)
    except Exception:
...
_SUMMER_LEAGUE_RE = re.compile(r"\bsummer\s*league\b|\bnba\s*sl\b", re.I)
...
def _score_result(bet, games, sport_key):
    ...
    cands = _strict_candidates(bet.get("description") or "", games)
    if not cands:
        return None, None
    g = cands[0]
    ...
    if btype == "moneyline":
        if sport_key in _DRAW_SPORTS:
            return None, None
        if both:
            mm = re.search(r"\b(ml|moneyline)\b", desc)
            team = _side_before(bet.get("description") or "", home, away,
                                mm.start()) if mm else None
            if not team:
                return None, None
            picked_home = (team == home)
        else:
            picked_home = hf_match
    ...
    if btype == "spread":
        ...
        point = float(m.group(1))
        if both:
            team = _side_before(bet.get("description") or "", home, away, m.start())
            if not team:
                return None, None
            picked_home = (team == home)
        else:
...
# ═══ NIGHTLY RECAP: one message after the slate, ~2am ET ═══════════════
_last_recap_date = {"d": None}


@tasks.loop(minutes=60)
async def nightly_recap():
    try:
        now_et = datetime.now(ZoneInfo("America/New_York"))
        if now_et.hour != 2 or _last_recap_date["d"] == now_et.date():
            return
        day_start_utc = now_et.replace(hour=0, minute=0, second=0, microsecond=0) \
            .astimezone(timezone.utc) - timedelta(days=0)
        # "today" = the ET day that just ended (recap runs at 2am for yesterday)
        y_start = (now_et - timedelta(days=1)).replace(hour=0, minute=0, second=0,
        ...
        async with aiosqlite.connect(DB_PATH) as db:
            ...
            rows = [dict(r) for r in await (await db.execute(
                ...
                (y_start.isoformat(), y_end.isoformat()))).fetchall()]
        _last_recap_date["d"] = now_et.date()
        if not rows:
            return
        w = sum(1 for r in rows if r["status"] == "won")
        l = sum(1 for r in rows if r["status"] == "lost")
        net = sum(r.get("profit") or 0 for r in rows)
        best = max(rows, key=lambda r: r.get("profit") or 0)
        worst = min(rows, key=lambda r: r.get("profit") or 0)
        clvs = [r["clv"] for r in rows if r.get("clv") is not None]
        lines = [f"**🌙 Daily Recap — {(now_et - timedelta(days=1)).strftime('%A %b %d')}**",
                 f"Record **{w}-{l}**  ·  Net **{'+' if net >= 0 else ''}${net:.2f}**"]
        if (best.get("profit") or 0) > 0:
            lines.append(f"Best: {(best.get('description') or '?')[:50]}  +${best['profit']:.2f}")
        if (worst.get("profit") or 0) < 0:
            lines.append(f"Worst: {(worst.get('description') or '?')[:50]}  -${abs(worst['profit']):.2f}")
        if clvs:
            avg = sum(clvs) / len(clvs)
            lines.append(f"CLV on {len(clvs)} closes: **{avg:+.1f} pts avg**")
        for grp in GROUPS:
            try:
        ...
        log.exception("nightly recap failed")


@nightly_recap.before_loop
async def before_nightly_recap():
    await bot.wait_until_ready()
...
async def score_watch():
    ...
    try:
        ...
        # ── misgrade detection on recently settled bets (warn-only;
        #    auto-grading was removed after a wrong-game grade) ──────────
        for b in recent:
            sk = _odds_sport_key(b.get("sport"), b.get("league"))
...
def _team_matches(team_name, desc_lower, desc_raw):
    """True if the description references this team by nickname, city name,
    or a known abbreviation ("LA", "CHI", "NYY"...). Generous by design --
    used for FINDING a candidate game and attributing sides. Conflict
    rejection uses the narrower _team_conflict_ref below."""
    frag = _team_frag(team_name)
    ...
    if city:
        if city in desc_lower:
            return True
        for abbr, c in _CITY_ABBR.items():
            if c == city and re.search(rf"\b{abbr}\b", desc_raw):
                return True
    return False


def _team_conflict_ref(team_name, desc_lower, desc_raw):
    """Narrow reference test for conflict rejection: nickname or explicit
    franchise abbreviation only. City-level matches are deliberately
    excluded -- 'Los Angeles' in a Lakers bet must not register as a
    reference to the Sparks and wrongly reject a legitimate match."""
    frag = _team_frag(team_name)
    if frag and re.search(rf"\b{re.escape(frag)}\b", desc_lower):
        return True
    for abbr, af in _MLB_ABBR_TO_FRAG.items():
        if af == frag and re.search(rf"\b{abbr}\b", desc_raw):
            return True
...
# ═══════════════════════════════════════════════════════════════════════
# STRICT SCHEDULE MATCH — the strongest league-detection signal available.
# Requires BOTH teams of a real scheduled game to be named in the bet text
# (not just one shared abbreviation), and prefers/filters by how close the
# game's start time is to when the bet was placed. A single-team or bare
# abbreviation match can never trigger this path, which is what let shared
# codes like MIN/LAA/SEA cause false corrections before. Cross-checked
# against the same nickname data that drives the logos, so what the bet
# gets labeled and what logo it shows can never disagree.
# ═══════════════════════════════════════════════════════════════════════
_STRICT_CANDIDATE_KEYS = ["basketball_nba", "basketball_wnba", "baseball_mlb",
                          "americanfootball_nfl", "icehockey_nhl",
                          # College leagues were defined in _KEY_TO_LEAGUE but
                          # missing here, so a college bet could never be matched
                          # by the schedule checks and always fell to guesswork.
                          "basketball_ncaab", "americanfootball_ncaaf"]


def _team_named_in(desc, team_name):
    """True if this real team is identifiable in the bet text — by
    distinctive nickname, city name, or the SAME abbreviation code that
    drives its logo (_ESPN_LOGO). Reusing the logo data here means a team
    can never be "found" for league-correction purposes without also being
    identifiable enough to show the right crest — league detection and
    logos are always sourced from one consistent dataset.

    A lone abbreviation is safe to accept here because this function is
    only ever called in pairs (both home AND away must hit) against one
    specific real scheduled game — a generic-looking code like "LA" can't
    trigger anything by itself; it only counts when paired with a real
    opponent for an actual game near the bet's date."""
    if not team_name:
        return False
    d = (desc or "").lower()
    frag = _team_frag(team_name)
    if frag and re.search(rf"\b{re.escape(frag)}\b", d):
        return True
    city = " ".join(team_name.split(" ")[:-1]).lower()
    if city and re.search(rf"\b{re.escape(city)}\b", d):
        return True
    # College teams have multi-word mascots ("Duke Blue Devils", "North
    # Carolina Tar Heels"), so the city slice above becomes "duke blue" /
    # "north carolina tar" and never matches a slip that just says "Duke".
    # Try progressively shorter leading portions of the name so the school
    # name itself matches. Requires 4+ chars to avoid trivial false hits.
    parts = team_name.split(" ")
    if len(parts) > 2:
        for take in range(len(parts) - 1, 0, -1):
            lead = " ".join(parts[:take]).lower()
            if len(lead) >= 4 and re.search(rf"\b{re.escape(lead)}\b", d):
                return True
    # Abbreviation match via the logo data. Find the ESPN nickname by
    # SUFFIX (not _team_frag's last-word-only lookup), so two-word
    # nicknames like "Red Sox" / "Blue Jays" resolve correctly too.
    tn = (team_name or "").lower()
    best_nick, best_len = None, -1
    for nick in _ESPN_LOGO:
        if tn.endswith(nick) and len(nick) > best_len:
            best_nick, best_len = nick, len(nick)
    if best_nick:
        abbr = _ESPN_LOGO[best_nick][1]
        if abbr and re.search(rf"\b{re.escape(abbr)}\b", desc or "", re.I):
            return True
    return False


def _both_teams_hit(desc, home, away):
    """True only when BOTH sides of one real game are named — the core of
    the strict match. A shared abbreviation naming just one team never
    satisfies this, no matter how distinctive it looks."""
    return _team_named_in(desc, home) and _team_named_in(desc, away)


_espn_nick_cache = {}


def _espn_nicknames_for_slug(slug):
    """Nicknames ESPN's logo data has for this league (mlb/nba/wnba) —
    reusing the exact list that drives the visual logos, so a league
    correction is never made for a team we couldn't even show a crest for."""
    if not _espn_nick_cache:
        for nick, (lg_slug, _abbr) in _ESPN_LOGO.items():
            _espn_nick_cache.setdefault(lg_slug, set()).add(nick)
    return _espn_nick_cache.get(slug, set())


def _logo_confirms(sport_key, home, away):
    """Sanity cross-check: at least one of the matched teams should also
    exist in the ESPN logo map for that league. Guards against the rare
    case of a schedule entry for a team we have no display data for."""
    slug = {"basketball_nba": "nba", "basketball_wnba": "wnba",
            "baseball_mlb": "mlb"}.get(sport_key)
    if not slug:
        return True   # NFL/NHL aren't in the logo map yet — don't block on it
    nicks = _espn_nicknames_for_slug(slug)
    if not nicks:
        return True   # logo map unavailable — don't block on it
    return (_team_frag(home) in nicks) or (_team_frag(away) in nicks)


def _strict_schedule_match(desc, created_at=None, candidate_keys=None):
    """The primary league-detection check. Returns a sport_key ONLY when
    exactly one league's real schedule has a game where BOTH teams are
    named in the bet text (optionally narrowed by how close the game's
    start time is to when the bet was placed). Returns None on zero or
    multiple such matches — genuine ambiguity is left alone, never guessed.
    """
    keys = candidate_keys or _STRICT_CANDIDATE_KEYS
    created = None
    if created_at:
        try:
    ...
    hits = []   # (sport_key, event)
    for key in keys:
        for e in _get_events(key):
            home, away = e.get("home_team"), e.get("away_team")
            if not (_both_teams_hit(desc, home, away) and _logo_confirms(key, home, away)):
                continue
            # Date is a HARD requirement, not a soft preference: up to 12h
            # after start (covers live/in-play bets) to 60h before (covers
            # early lines). Both teams matching isn't enough by itself if
            # the game they matched happened on a totally different date —
            # that's a coincidental pairing, not this bet's actual game.
            ct = e.get("commence_time")
            if ct:
                try:
                    commence = datetime.fromisoformat(str(ct).replace("Z", "+00:00"))
                    delta_h = (commence - created).total_seconds() / 3600
                    if not (-12 <= delta_h <= 60):
                        continue   # right teams, wrong date — not a match
                except (ValueError, TypeError):
                    pass   # can't parse the date — fall through, keep it
            hits.append((key, e))
    ...
    return None   # multiple leagues' schedules both fit — too ambiguous to guess



def _single_team_schedule_match(desc, created_at=None, candidate_keys=None):
    """Second-strongest evidence tier: exactly ONE league's real schedule
    has a game where a team is named in the bet text.

    This fills a structural blind spot. _strict_schedule_match requires BOTH
    teams, so a very common slip format ("Bears +3.5", "Storm ML") falls
    through it entirely — and then lands on weak heuristics that TRUST the
    parsed sport and can confidently make a wrong label worse (e.g. an NFL
    bet misparsed as Basketball/NBA getting "corrected" to WNBA by the
    season backstop, because nothing ever questioned "Basketball" itself).

    Matching against the live schedule rather than a static nickname list
    also means every league the API covers is protected — including NFL and
    NHL, which aren't in the logo map at all.

    Strict about ambiguity: if teams from two different leagues both match,
    or the same league matches games too far from the bet's date, it returns
    ...
    for key in keys:
        for e in _get_events(key):
            home, away = e.get("home_team"), e.get("away_team")
            if not (_team_named_in(desc, home) or _team_named_in(desc, away)):
                continue
            ct = e.get("commence_time")
            if ct:
                try:
                    ...
                    delta_h = (commence - created).total_seconds() / 3600
                    if not (-12 <= delta_h <= 60):
                        continue   # right team, wrong date window
                except (ValueError, TypeError):
                    pass
            matched_keys.add(key)
            break   # one confirmed game per league is enough

    if len(matched_keys) == 1:
        return matched_keys.pop()
    return None   # zero matches, or genuinely ambiguous across leagues



def _schedule_was_ambiguous(desc, created_at=None, candidate_keys=None):
    """True when MORE THAN ONE league's real schedule has a game with both
    teams named in the bet text. In that case the evidence genuinely does
    not identify a league, and every weaker heuristic downstream would be
    guessing — so callers should stop rather than let a static abbreviation
    list pick a winner by coincidence."""
    keys = candidate_keys or _STRICT_CANDIDATE_KEYS
    created = None
    if created_at:
        try:
    ...
    # A match on a distinctive NICKNAME ("Storm", "Lynx", "Mariners") is
    # decisive — nicknames are unique across leagues. Only matches resting
    # purely on shared cities/abbreviations ("SEA", "MIN") can be
    # ambiguous, so track the two kinds separately.
    def _nickname_hit(team):
        frag = _team_frag(team)
        return bool(frag) and re.search(rf"\b{re.escape(frag)}\b",
                                        (desc or "").lower()) is not None

    matched = set()
    nickname_matched = set()
    for key in keys:
        for e in _get_events(key):
            home, away = e.get("home_team"), e.get("away_team")
            if not _both_teams_hit(desc, home, away):
                continue
            if not _logo_confirms(key, home, away):
                continue
            if _nickname_hit(home) or _nickname_hit(away):
                nickname_matched.add(key)
            ct = e.get("commence_time")
            if ct:
                try:
                    commence = datetime.fromisoformat(str(ct).replace("Z", "+00:00"))
            ...
            matched.add(key)
            break
    # exactly one league matched on a real nickname -> not ambiguous
    if len(nickname_matched) == 1:
        return False
...
def verify_league(data):
    ...
    try:
        ...
        # ── PRIMARY CHECK: strict schedule match ──────────────────────
        # If a real game has BOTH teams named in the bet text, that's the
        # strongest evidence available — far stronger than one team name
        # or a shared abbreviation. Try this before any weaker heuristic.
        # The sport-vocabulary guard must run BEFORE any schedule matching.
        # Shared abbreviations (MIN/SEA/LAA) belong to teams in several
        # leagues, so an MLB prop like "Over 6.5 Strikeouts Thrown in MIN
        # vs SEA" can satisfy a WNBA both-teams match purely on codes. An
        # explicit baseball/football stat term in the text is stronger
        # evidence than an abbreviation coincidence, so it wins outright.
        blocking_sport = _nonbasketball_sport_from_desc(desc)
        allowed_keys = None
        if blocking_sport == "baseball":
            allowed_keys = ["baseball_mlb"]
        elif blocking_sport == "football":
            allowed_keys = ["americanfootball_nfl", "americanfootball_ncaaf"]

        strict_key = _strict_schedule_match(desc, data.get("created_at"),
                                            candidate_keys=allowed_keys)
        if strict_key:
            claimed_key = _odds_sport_key(data.get("sport"), data.get("league"))
            if claimed_key != strict_key:
                new_sport, new_league = _KEY_TO_LEAGUE[strict_key]
                old_sport = data.get("sport") or "?"
                old_league = data.get("league") or "?"
                data["sport"], data["league"] = new_sport, new_league
                note = (f"league corrected: {old_sport}/{old_league} → "
                       f"{new_sport}/{new_league} (both teams matched vs schedule)")
                log.info(note + f" | {desc[:60]}")
                return data, note
            return data, None   # parse already agrees with the strict match — done

        # ── SECOND CHECK: single-team schedule match ──────────────────
        # Slips very often name only one side ("Bears +3.5", "Storm ML"),
        # which the both-teams check above can't use. Matching one team
        # against exactly one league's live schedule is still far stronger
        # than the static-list heuristics below — and crucially it verifies
        # the SPORT itself, instead of trusting a possibly-wrong parse and
        # then "correcting" the league underneath it.
        # Distinguish "no schedule evidence" from "schedule evidence was
        # AMBIGUOUS". If two leagues both had a real matching game, the
        # strict matcher deliberately refused — the weaker static-list
        # heuristics below must NOT then guess anyway, or a shared
        # abbreviation (SEA/MIN belong to Mariners/Twins AND Storm/Lynx)
        # gets resolved by coincidence instead of evidence.
        if _schedule_was_ambiguous(desc, data.get("created_at"), allowed_keys):
            log.info(f"league ambiguous across leagues — left as parsed | {desc[:60]}")
            return data, None

        single_key = _single_team_schedule_match(desc, data.get("created_at"),
                                                 candidate_keys=allowed_keys)
        if single_key:
            claimed_key = _odds_sport_key(data.get("sport"), data.get("league"))
            if claimed_key != single_key:
                new_sport, new_league = _KEY_TO_LEAGUE[single_key]
                old_sport = data.get("sport") or "?"
                old_league = data.get("league") or "?"
                data["sport"], data["league"] = new_sport, new_league
                note = (f"league corrected: {old_sport}/{old_league} → "
                       f"{new_sport}/{new_league} (team matched vs schedule)")
                log.info(note + f" | {desc[:60]}")
                return data, note
            return data, None   # parse agrees with schedule evidence — done

        # ── FALLBACK: weaker heuristics, only when the schedule gave no
        # usable evidence at all (API down, off-season, or a bet naming no
        # recognizable team). These are unchanged from before. ──────
        ...
        if sport != "basketball":
            ...
            by_team = None if blocking_sport else _basketball_league_from_desc(desc)
            if by_team:
                old_sport = data.get("sport") or "?"
                old_league = data.get("league") or "?"
        ...
        if sport == "basketball":
            if _SUMMER_LEAGUE_RE.search(desc):
                return data, None   # NBA Summer League in July is correctly NBA
            ...
            # Guarded — this flips NBA<->WNBA purely on the calendar, so if
            # the SPORT itself was misparsed (e.g. an NFL bet labeled
            # Basketball), applying it would confidently make a wrong label
            # worse. Skip whenever the text names a non-basketball stat.
            if _nonbasketball_sport_from_desc(desc):
                return data, None
...
async def _prop_closing_pass(b, sk, now):
    """Closing-line capture for one pending prop bet, mirroring the game-bet
    flow: capture only in the true pre-game window, sentinel when missed."""
    try:
        if not sk or b.get("odds") is None:
            return
        eid, commence = b.get("event_id"), b.get("game_time")
        desc = b.get("description") or ""
        market = _prop_market_key(sk, b.get("prop_category"), desc)
        player, side, point = _parse_prop(desc)
        if not market or not player or point is None:
            return
        ...
        if not eid:
            # log-time resolution failed or predates the feature — one
            # bounded retry per poll while the bet is <36h old
            eid, commence, _ = await _resolve_prop_event(sk, market, player,
                                                         max_tries=2)
            if not eid:
                return
            async with aiosqlite.connect(DB_PATH) as db:
                await db.execute(
                    ...
                    (eid, commence, mid))
                await db.commit()
            await sheets_update_odds(uname, mid, game_time=commence, background=True)
        ...
        if mins_out <= -5:
            async with aiosqlite.connect(DB_PATH) as db:
                ...
                await db.commit()
            log.info(f"prop CLV missed (game started) for {mid}")
            return
        if mins_out > ODDS_POLL_MINUTES:
            return   # not in the closing window yet; no spend

        data = await asyncio.get_event_loop().run_in_executor(
            None, _fetch_event_prop_odds, sk, eid, market)
        price = _prop_price(data, market, player, side, point)
        if price is None:
            return   # next poll may still land inside the window
        ...
        async with aiosqlite.connect(DB_PATH) as db:
            await db.execute(
                ...
                (price, clv_pts, mid))
            await db.commit()
        await sheets_update_odds(uname, mid, closing=price, clv=clv_pts, background=True)
        log.info(f"prop CLV captured {mid}: entry {entry} close {price} -> {clv_pts:+.1f}")
    except Exception:
        log.exception("prop closing pass failed")
    ...
    try:
        async with aiosqlite.connect(DB_PATH) as db:
            ...
            cur = await db.execute(
                ...
                "AND lower(bet_type) IN ('moneyline','total','spread','prop')")
        ...
        for b in bets:
            sk = _odds_sport_key(b.get("sport"), b.get("league"))
            if (b.get("bet_type") or "").lower() == "prop":
                await _prop_closing_pass(b, sk, now)
                continue
            ...
            if not commence:
                continue   # game itself not found at all -- nothing to persist
            try:
                start = datetime.fromisoformat(commence.replace("Z", "+00:00"))
            except Exception:
                continue
            ...
            # Persist the game start time whenever we've identified the game,
            # independent of whether a price was found. This is what powers
            # the Live section -- a game with no bookmaker price posted right
            # at start (common in-play) must still be recorded as live.
            if b.get("game_time") != commence:
                async with aiosqlite.connect(DB_PATH) as db:
                    await db.execute("UPDATE bets SET game_time=? WHERE message_id=?",
                                     (commence, mid))
                    await db.commit()
            ...
            if price is None:
                # game found, but no current price (e.g. book suspended the
                # market right at start) -- game_time is saved above, but
                # there's nothing further to do (no CLV/live-odds/alert data)
                continue
            ...
            if mins_out <= ODDS_POLL_MINUTES:
                ...
                # ── live odds refresh ───────────────────────────────
                # This is the highest-VOLUME sheet write in the bot: it can
                # fire for every pending bet on every poll cycle. It's also
                # the lowest-VALUE one — a cosmetic "current price" reading
                # that's superseded on the next cycle anyway, unlike the
                # closing/CLV capture above which is permanent and matters.
                # Always record it in the DB (free), but only spend a Sheets
                # write when the price actually moved enough to be worth
                # seeing. Small ticks aren't worth the quota, especially
                # since that quota is shared with grading.
                if b.get("live_odds") != price:
                    prev = b.get("live_odds")
                    async with aiosqlite.connect(DB_PATH) as db:
                        await db.execute(
                        ...
                        await db.commit()
                    try:
                        moved = abs(_implied(price) - _implied(prev)) * 100 if prev else 99
                    except (TypeError, ValueError):
                        moved = 99
                    if moved >= LIVE_ODDS_MIN_MOVE_PTS:
                        await sheets_update_odds(uname, mid, live=price, background=True)
...
@bot.tree.command(name="headshots", description="Refresh the ESPN player-photo index, or test a player name")
@app_commands.describe(test_name="Optional: check which player a name resolves to")
async def headshots(interaction: discord.Interaction, test_name: str = None):
    await interaction.response.defer(ephemeral=True)
    if test_name:
        lines = [f"**Resolving `{test_name}`**"]
        for sk in _ESPN_LEAGUE_PATH:
            p = _lookup_player(sk, test_name)
            lg = sk.split("_")[-1].upper()
            if p:
                lines.append(f"• {lg}: **{p['full']}** (id {p['id']})")
            else:
                loaded = len(_PLAYER_INDEX.get(sk) or {})
                lines.append(f"• {lg}: no unambiguous match"
                             + ("" if loaded else "  _(index empty — run /headshots)_"))
        await interaction.followup.send("\n".join(lines), ephemeral=True)
        return

    await interaction.followup.send("Refreshing ESPN player index…", ephemeral=True)
    out = []
    for sk in _ESPN_LEAGUE_PATH:
        try:
            n = await refresh_player_index(sk, force=True)
            loaded = len(_PLAYER_INDEX.get(sk) or {})
            lg = sk.split("_")[-1].upper()
            out.append(f"• {lg}: {n if n > 0 else 0} athletes fetched, "
                       f"{loaded} surnames indexed")
        except Exception as e:
            out.append(f"• {sk}: failed ({type(e).__name__})")
    await interaction.followup.send(
        "**🖼️ Player index refreshed**\n" + "\n".join(out)
        + "\n\nProps logged from now on get the player's photo on their bet card.",
        ephemeral=True)
    ...
    async def _run():
        ...
        for (sk, day), day_bets in sorted(by_day.items()):
            ...
            for b in day_bets:
                ...
                cands = _strict_candidates(desc_raw, ev_list)
                if len(cands) == 0:
                    # distinguish true no-match from conflict-rejected by retrying loose
                    loose = [e for e in ev_list
                             if _team_matches(e.get("home_team"), desc, desc_raw)
                             or _team_matches(e.get("away_team"), desc, desc_raw)]
                    cands = loose if len(loose) > 1 else []
                    if not cands:
                ...
                if len(cands) == 1:
                    ...
                    if ev is None:
                        ...
                        continue
                # game matched — save its start time regardless of whether the
                # closing-line steps below succeed (powers Entry Timing lead-time)
                _commence = ev.get("commence_time")
                if _commence and b.get("game_time") != _commence:
                    async with aiosqlite.connect(DB_PATH) as db:
                        await db.execute(
                            "UPDATE bets SET game_time=? WHERE message_id=?",
                            (_commence, b["message_id"]))
                        await db.commit()
                    times_saved["count"] += 1
                eid = ev["id"]
                ...
                if btype == "moneyline":
                    if not both:
                        ...
                        mm = re.search(r"\b(ml|moneyline)\b", desc)
                        team = _side_before(desc_raw, ev["home_team"], ev["away_team"],
                                            mm.start()) if mm else None
                        if team:
                            pick = ("h2h", team)
                elif btype == "total":
                    ...
                    if m:
                        team = None
                        if not both:
                            team = ev["home_team"] if hf_match else ev["away_team"]
                        else:
                            team = _side_before(desc_raw, ev["home_team"],
                                                ev["away_team"], m.start())
                        if team:
                            pick = ("spreads", (team, float(m.group(1))))
                if pick is None:
        ...
        gt_note = (f"🕐 {times_saved['count']} game times saved (Entry Timing lead-time data)\n"
                  if times_saved["count"] else "")
        sample_txt = ""
...
async def fixleagues(interaction: discord.Interaction):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        for b in rows:
            ...
            # PRIMARY: strict schedule match using this bet's real placement
            # date — the strongest evidence available for historical bets too.
            strict_key = _strict_schedule_match(b.get("description"), b.get("created_at"))
            if strict_key:
                claimed_key = _odds_sport_key(b.get("sport"), b.get("league"))
                if claimed_key != strict_key:
                    new_sport, new_league = _KEY_TO_LEAGUE[strict_key]
                    await db.execute(
                        "UPDATE bets SET sport=?, league=? WHERE message_id=?",
                        (new_sport, new_league, b["message_id"]))
                    changes.append(f"[schedule: both teams] {claimed_sport or '?'}/{claimed or '?'} → "
                                   f"{new_sport}/{new_league}: {(b.get('description') or '')[:45]}")
                continue   # strict match settled it either way — skip fallback heuristics
    ...
    async def _do_resync():
        ...
        try:
            ...
            if not SHEETS_AVAILABLE or not SPREADSHEET_ID:
                ...
                await _call_sheets(_rewrite_sheets, cost=_SHEETS_COST["bulk"])
