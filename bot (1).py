...
async def post_period_stats(channel, days, label, group_name=None):
...
async def periodic_stats():
    ...
    do_daily = now.hour == 0 and _last_daily != today
    do_weekly = now.weekday() == 0 and now.hour == 0 and _last_weekly != today
    do_monthly = now.day == 1 and now.hour == 0 and _last_monthly != today
    ...
    for g in GROUPS:
        ...
        if do_daily:
            await post_period_stats(channel, days=1, label=f"Daily ({name})", group_name=name)
        if do_weekly:
            await post_period_stats(channel, days=7, label=f"Weekly ({name})", group_name=name)
        if do_monthly:
            await post_period_stats(channel, days=30, label=f"Monthly ({name})", group_name=name)
...
async def on_ready():
    ...
    if any(g.get("stats_channel_id") for g in GROUPS) and not periodic_stats.is_running():
...
async def recap(interaction: discord.Interaction, period: app_commands.Choice[str]):
    ...
    label = f"{label_map[period.value]} ({group_name})"

    stats_id = group.get("stats_channel_id") if group else None
    target_channel = bot.get_channel(int(stats_id)) if stats_id else interaction.channel
    await post_period_stats(target_channel, days=days, label=label, group_name=group_name)

    if stats_id and target_channel.id != interaction.channel.id:
