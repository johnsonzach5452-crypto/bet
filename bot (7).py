...
async def audit(interaction: discord.Interaction):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        for bet in bets:
            ...
            if correct is None:
                # Still can't calculate — missing both odds and payout
    ...
    if null_profit:
        lines.append(f"\n**{len(null_profit)} bet(s) still can't be calculated** (no odds or payout on record):")
        lines.extend(f"• {d}" for d in null_profit)
    ...
    msg = "\n".join(lines)
    if len(msg) > 1900:
        msg = msg[:1900] + f"\n... and more. {len(fixed)} total fixed."
        ...
        if stats_channel:
            ...
            if null_profit:
                audit_embed.add_field(
                    ...
                    value=f"{len(null_profit)} bet(s) have no odds or payout on record — profit stays at 0.",
                    inline=False,
                )
